...
### currentFrameCallback(frameCount) ###

This callback will be triggered each time a new frame is drawn.  Your function declaration should include one parameter, which will be passed the current framecount.  This count will reset to zero each time the composition repeats.
